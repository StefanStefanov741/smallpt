//g++ -std=c++11 -O3 -pthread smallptPthreads.cpp -o smallptPthreads
...
struct Vec {
  ...
  Vec operator[](short unsigned int i) const { 
  return i == 0 ? Vec(x,0,0) : (i == 1 ? Vec(0,y,0) : Vec(0,0,z));
  }
...
void *render(void *arg) {
    ...
    for (int y = threadNum; y < h; y += NUM_THREADS) {
        //Loop over columns
...
int main(int argc, char *argv[]) {
    ...
    //Call render function for each thread
    pthread_t threads[NUM_THREADS];
    int threadNums[NUM_THREADS];
    for (int i = 0; i < NUM_THREADS; i++) {
        threadNums[i] = i;
        pthread_create(&threads[i], NULL, render, &threadNums[i]);
    }
