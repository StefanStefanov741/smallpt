//mpic++ -o smallptMPI smallptMPI.cpp
//mpiexec -n 6 ./smallptMPI
...
int main(int argc, char *argv[]) {
  ...
  int rowsPerProc = h / nodesCount; //number of rows per process
  int leftoverRows = h % nodesCount; //any remaining rows after dividing evenly

  //Determine the starting row for each process
  int startRow = nodeID * rowsPerProc;

  if (nodeID < leftoverRows) {
      //Add extra work to the first few processes to take the leftover rows
      startRow += nodeID;
  }

  //Determine the number of rows to process for this process
  int numRows = rowsPerProc;
  if (nodeID < leftoverRows) {
      //Add extra work to the first few processes to take the leftover rows
      numRows += 1;
  }
  ...
  Vec* localC = new Vec[w * numRows];

  //Loop over image rows for each process
    ...
    for (unsigned short x = 0, Xi[3] = {0, 0, y * y * y}; x < w; x++) {
      ...
      for (int sy = 0, i = (h - y - 1) * w + x; sy < 2; sy++) {
        ...
        for (int sx = 0; sx < 2; sx++, r = Vec()) {
          ...
          localC[(y - startRow) * w + x] = localC[(y - startRow) * w + x] + Vec(clamp(r.x), clamp(r.y), clamp(r.z)) * 0.25;
        }
      }
    }
  }
  ...
  if (nodeID == 0) {
    ...
    //Copy the local canvas from node 0 to the final canvas
    std::copy(localC, localC + numRows * w, final_canvas);

    //Receive partial canvases from the other nodes and store them in the final canvas
    for (int i = 1; i < nodesCount; i++) {
      MPI_Recv(final_canvas + i * numRows * w, numRows*3 * w, MPI_DOUBLE, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE); //Adding the *3 fixed the issue with only 1/3 of canvas being recieved
    }
    ...
    //Send partial canvas to node 0
    MPI_Send(localC, numRows * w *3, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
